      ...
      <div className="relative">
        ...
        {showPopup && (
          <div className="absolute px-2 right-0 top-10 bg-white shadow-md rounded-lg  py-2 mt-4 min-w-[12rem] z-50">
            ...
            <Link
              to="/wishlist"
              className="block relative flex items-center gap-2 px-3 text-sm py-2 rounded-lg hover:bg-gray-200"
            >
              <AiOutlineHeart size={17} className='text-pink-500' />
              Wishlist
              {user?.wishlist.length > 0 &&
                <span class="relative flex h-3 w-3">
                  <span class="animate-ping absolute bottom-2 left-2 inline-flex h-full w-full rounded-full bg-pink-400 opacity-75"></span>
                  <span class="relative inline-flex bottom-2 left-2 rounded-full h-3 w-3 bg-pink-500"></span>
                </span>
              }
            </Link>
            <a
